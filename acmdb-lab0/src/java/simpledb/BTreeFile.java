...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
        ...
        if(f==null) {
        	...
        	BTreePageId chileId=page.iterator().next().getLeftChild();
        	return findLeafPage(tid, dirtypages, chileId,perm, f);
        }
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
        ...
        int tupleSum=page.getNumTuples();
        Iterator<Tuple> it=page.reverseIterator();
        for(int i=0;i<tupleSum/2;++i) {
        	Tuple tuple=it.next();
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int entriesSum=page.getNumEntries();
		Iterator<BTreeEntry> it=page.reverseIterator();
		for(int i=0;i<entriesSum/2;++i) {
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int pageTupleSum=page.getNumTuples(),siblingTupleSum=sibling.getNumTuples();
		int midTupleSum=(pageTupleSum+siblingTupleSum)/2;
		while(pageTupleSum<midTupleSum) {
		...
		Tuple headTuple=siblingit.next();
		entry.setKey(headTuple.getField(keyField));
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeEntry itEntry=leftIterator.next();
		...
		int pageEntrySum=page.getNumEntries();
		int siblingEntrySum=leftSibling.getNumEntries();
		int midEntrySum=(pageEntrySum+siblingEntrySum)/2;
		while(pageEntrySum<midEntrySum) {
			leftSibling.deleteKeyAndRightChild(itEntry);
			page.insertEntry(itEntry);
			pageEntrySum++;
			itEntry=leftIterator.next();
		}
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeEntry itEntry=rightIterator.next();
